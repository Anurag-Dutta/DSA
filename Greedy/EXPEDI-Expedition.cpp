...
On this road, between the town and the current location of the truck, there are N (1 <= N <= 10,000) fuel stops
...
Fortunately, the capacity of the fuel tank on their truck is so large that there is effectively
no limit to the amount of fuel it can hold. The truck is currently L units away from the town
and has P units of fuel (1 <= P <= 1,000,000).
...
int main(int argc, char **argv)
{
    ...
    {
        int n = 4;
        vector<vector<int>> details = {{4, 4}, {5, 2}, {11, 5}, {15, 10}};
        int dist_initial = 25, fuel_initial = 10;
        int more_fuel_needed = dist_initial - fuel_initial;
        for (int i = 0; i < details.size(); i++)
        {
            details[i][0] = 25 - details[i][0];
        }
        for (int i = 0; i < details.size(); i++)
        {
            swap(details[i][0], details[i][1]);
        }
        ...
        int init = 0;
        int stop = 0;
        while (more_fuel_needed != 0)
        {
            if (more_fuel_needed >= details[init][1])
            {
                more_fuel_needed -= details[init][1];
                stop++;
                init++;
            }
            else
